...
# Script for TSP-200 Hardness-Adaptive Fine-Tuning and Saving

import sys
import os
os.environ['PYTORCH_CUDA_ALLOC_CONF'] = 'expandable_segments:True' # Mitigate CUDA OOM
...
# --- Configuration ---
print("Setting up configuration for TSP-200 Hardness-Adaptive Fine-Tuning...")
opts = get_options('') # Load default options first

# --- Override options for TSP-200 fine-tuning run ---
opts.problem = 'tsp'
opts.graph_size = 200 # <<<--- Set graph size to 200
opts.use_cuda = True
opts.no_progress_bar = False
opts.log_dir = 'logs/tsp200_finetuned_adaptive' # Specific log directory for fine-tuning
opts.save_plot = 'tsp200_finetuned_adaptive_curve.png' # Specific plot filename for fine-tuning
opts.save_dir = 'new_main' # Reuse the save directory
opts.finetuned_model_save_filename = 'tsp200_finetuned_adaptive.pt' # Specific model save name for fine-tuning
opts.finetuned_args_save_filename = 'args_tsp200_finetuned_adaptive.json' # Specific args save name for fine-tuning

# --- Pre-trained model path ---
opts.load_path = 'checkpoints/tsp200_finetuned_adaptive/tsp200_checkpoint_epoch_70.pt' # <<<--- Load the fine-tuned checkpoint for resuming
opts.baseline = 'rollout'

# Hardness-Adaptive specific settings (should be already set)
opts.reweight = 1
# use_hard_setting = 1 # Removed as not used in tsp200
...
opts.batch_size = 128 # <<<--- Reduced for TSP-200 due to OOM
...
opts.eval_batch_size = 256 # <<<--- Explicitly set and reduced for TSP-200 due to OOM

# Hardness sampling parameters
opts.hardness_eps = 5
opts.hardness_train_size = 10000

# --- ASSUMED DATA PATHS FOR TSP-200 (Ensure these files exist!) ---
# These should be the same as used in the uniform pretraining script
opts.val_dataset = 'data/tsp/tsp200_val_mg_seed2222_size10K.pkl' # <<<--- Updated validation data path based on list_files
opts.val_gt_path = 'data/tsp/tsp200_val_mg_seed2222_size10K_lkh_costs.txt' # <<<--- Updated LKH costs path based on list_files
# Base training data path used by init()
opts.train_dataset = 'data/tsp/tsp200_train_seed1111_size10K.pkl' # <<<--- Updated training data path based on list_files
# ------------------------------------------------------------------
...
try:
    ...
    print(f"Error: Could not find validation data file: {e}. Please ensure TSP-200 validation data exists.")
    val_dataset = None
...
def train_batch_adaptive(
    ...
    return cost.mean().detach().item(), total_loss.mean().detach().item()
...
if __name__ == '__main__':
    ...
    plot_data.append((start_epoch, initial_metrics[2])) # Record cost at the starting epoch
    ...
    for epoch in range(start_epoch, opts.n_epochs):
        ...
        if (epoch + 1) % checkpoint_epoch_frequency == 0 and (epoch + 1) < opts.n_epochs:
            checkpoint_save_dir = os.path.join('checkpoints', 'tsp200_finetuned_adaptive') # Use a specific checkpoint directory
            os.makedirs(checkpoint_save_dir, exist_ok=True)
            checkpoint_path = os.path.join(checkpoint_save_dir, f'tsp200_checkpoint_epoch_{epoch + 1}.pt')
