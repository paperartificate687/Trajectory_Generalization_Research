...
# Script for TSP-150 Hardness-Adaptive Fine-Tuning and Saving
...
# --- Configuration ---
print("Setting up configuration for TSP-150 Hardness-Adaptive Fine-Tuning...")
opts = get_options('') # Load default options first

# --- Override options for TSP-150 fine-tuning run ---
opts.problem = 'tsp'
opts.graph_size = 150 # <<<--- Set graph size to 150
opts.use_cuda = True
opts.no_progress_bar = False
opts.log_dir = 'logs/tsp150_finetuned_adaptive' # Specific log directory for fine-tuning
opts.save_plot = 'tsp150_finetuned_adaptive_curve.png' # Specific plot filename for fine-tuning
opts.save_dir = 'new_main' # Reuse the save directory
opts.finetuned_model_save_filename = 'tsp150_finetuned_adaptive.pt' # Specific model save name for fine-tuning
opts.finetuned_args_save_filename = 'args_tsp150_finetuned_adaptive.json' # Specific args save name for fine-tuning

# --- Pre-trained model path ---
opts.load_path = 'new_main/tsp150_uniform_pretrained.pt' # <<<--- Load the model trained by the previous script

# --- Pre-trained model path ---
opts.load_path = 'new_main/tsp150_uniform_pretrained.pt' # <<<--- Load the pre-trained uniform model
opts.baseline = 'rollout'

# Hardness-Adaptive specific settings (should be already set)
opts.reweight = 1
# use_hard_setting = 1 # Removed as not used in tsp150
...
opts.batch_size = 128 # <<<--- Reduced for TSP-150 due to OOM
...
opts.eval_batch_size = 256 # <<<--- Explicitly set and reduced for TSP-150 due to OOM

# Hardness sampling parameters
opts.hardness_eps = 5
opts.hardness_train_size = 10000

# --- ASSUMED DATA PATHS FOR TSP-150 (Ensure these files exist!) ---
# These should be the same as used in the uniform pretraining script
opts.val_dataset = 'data/tsp/tsp150_val_mg_seed2222_size10K.pkl' # <<<--- Updated validation data path based on list_files
opts.val_gt_path = 'data/tsp/tsp150_val_mg_seed2222_size10K_lkh_costs.txt' # <<<--- Updated LKH costs path based on list_files
# Base training data path used by init()
opts.train_dataset = 'data/tsp/tsp150_train_seed1111_size10K.pkl' # <<<--- Updated training data path based on list_files
# ------------------------------------------------------------------
...
try:
    ...
    print(f"Error: Could not find validation data file: {e}. Please ensure TSP-150 validation data exists.")
    val_dataset = None
