...
# New script for TSP-50 Unified Dataset Training and Saving
...
# --- Configuration ---
print("Setting up configuration for TSP-50 Unified Dataset Training...")
...
opts.log_dir = 'logs/tsp50_unified_trained' # Specific log directory for this run
opts.save_dir = 'new_main' # Directory to save model and args
opts.model_save_filename = 'tsp50_unified_trained.pt'
opts.args_save_filename = 'args_tsp50_unified_trained.json'
opts.plot_save_filename = 'tsp50_unified_curve.png' # Added plot filename

# Checkpoint saving configuration
opts.checkpoint_save_dir = 'checkpoints/tsp50_unified_trained' # Directory to save checkpoints
...
# Example: python src/generate_data.py --problem tsp --graph_sizes 50 --name train_unified --seed 5678 --dataset_size 10000 --generate_type unified
opts.train_dataset = 'data/tsp/tsp50_train_unified_seed5678_size10K.pkl' # Adjust seed and size as needed

opts.val_dataset = 'data/tsp/tsp50_val_mg_seed2222_size10K.pkl' # Keep validation dataset consistent or use a unified val set
...
try:
    ...
    print(f"Please generate it first using: python src/generate_data.py --problem tsp --graph_sizes 50 --name train_unified --seed YOUR_SEED --dataset_size YOUR_SIZE --generate_type unified -f")
    exit(1)
...
# --- Helper Functions (Adapted/Copied from train_and_save_tsp50.py) ---
...
def train_epoch_unified(model, optimizer, baseline, opts, train_dataset_to_use, epoch):
    print(f"===== Starting Epoch {epoch+1}/{opts.n_epochs} for Unified Dataset =====")
    ...
    pbar = tqdm(total=num_batches_to_process, desc=f"Epoch {epoch+1} Training (Unified)", leave=False, disable=opts.no_progress_bar)
    ...
    print(f"Epoch {epoch+1} completed. Avg Train Cost (Unified): {avg_cost:.4f}, Avg Train Loss: {avg_loss:.4f}")
...
if __name__ == '__main__':
    print("Initializing model, baseline, optimizer for training with Unified dataset...")
    # The init function from mutils.py will be used.
    # It loads a default dataset specified by its internal logic (e.g. tsp50_train_seed1111_size10K.pkl)
    # This is okay for initializing the model structure and optimizer.
    # The actual training will use `train_dataset_unified` loaded above.
    ...
    print(f"===== Starting Unified Dataset Training from epoch {start_epoch + 1} to {opts.n_epochs} =====")
    start_time_total = time.time()
    
    for epoch in range(start_epoch, opts.n_epochs):
        epoch_start_time = time.time()
        
        print(f"Epoch {epoch+1}: Using **Unified** training dataset of size {len(train_dataset_unified)}")
    ...
    print(f"===== Unified Training Finished (Reached epoch {opts.n_epochs}) =====")
    ...
    print(f"Saving final model (trained on Unified) to: {final_model_save_path}")
    torch.save({
        'epoch': opts.n_epochs, 
        'model_state_dict': get_inner_model(model).state_dict(),
        'optimizer_state_dict': optimizer.state_dict(),
    ...
    print(f"Saving training arguments (Unified) to: {final_args_save_path}")
    opts_dict = vars(opts) 
    ...
    print("===== Unified Training Script Finished =====") 
    
    print("Generating training curve plot (Unified)...")
    if plot_data:
