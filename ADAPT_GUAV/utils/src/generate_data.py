...
from src.mutils import generate_tsp_data_mg, generate_tsp_data_unified
...
if __name__ == "__main__":
    ...
    parser.add_argument('--generate_type', type=str, required=True, choices=['mg', 'uniform', 'unified'], help="mg, uniform, or unified")
            ...
            for graph_size in opts.graph_sizes:
                ...
                if problem == 'tsp':
                    if opts.generate_type == 'mg':
                        dataset = generate_tsp_data_mg(opts.dataset_size, graph_size)
                    elif opts.generate_type == 'uniform':
                        dataset = generate_tsp_data_uniform(opts.dataset_size, graph_size)
                    elif opts.generate_type == 'unified':
                        dataset = generate_tsp_data_unified(opts.dataset_size, graph_size)
                    else: # Should not happen due to choices in argparser
                        raise ValueError(f"Unknown generate_type: {opts.generate_type}")
                elif problem == 'vrp':
