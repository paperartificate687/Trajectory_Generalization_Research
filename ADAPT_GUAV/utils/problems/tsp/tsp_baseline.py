...
def solve_gurobi(directory, name, loc, disable_cache=False, timeout=None, gap=None, runs=1):
    """
    Solve TSP using LKH solver. 
    The function name is kept as solve_gurobi for compatibility with existing scripts,
    but it now calls the LKH solver logic.
    The timeout and gap parameters are currently ignored as LKH control is primarily via .par file (runs parameter added).
    """
    # Use the user-provided LKH executable path directly
    lkh_executable = r"C:\\Users\\Administrator\\Documents\\TSP-HAC-main\\TSP-HAC-main\\LKH-3.0.13\\LKH.exe"
    
    # Use solve_lkh_log for solving, reusing its caching mechanism
    # Note: solve_lkh_log uses its own specific file naming for caching based on runs
    try:
        # We call solve_lkh_log which handles its own caching.
        # The cache filename inside solve_lkh_log will be based on '.lkh{}.pkl'.format(runs)
        # We pass disable_cache to solve_lkh_log.
        cost, tour, duration = solve_lkh_log(
        ...
        print(f"Exception occurred while running LKH for {name}: {e}")
        # You might want to return a specific structure or raise an exception
        # depending on how the calling code handles failures.
        return None
