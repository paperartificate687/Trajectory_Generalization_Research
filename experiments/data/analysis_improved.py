...
def create_qos_visuals(data_path, output_dir):
    ...
    This script contains enhanced visual settings for better clarity in dual-column paper formats.
    ...
    try:
        plt.rcParams.update({
            'font.family': 'Times New Roman', 'font.size': 14, 'axes.labelsize': 14,
        ...
        for i, scenario in enumerate(scenarios):
            ...
            if not best_performer.empty:
                ...
                try:
                    ...
                    ax.text(x_pos, height + (ax.get_ylim()[1] * 0.01), '*', ha='center', va='bottom', fontsize=20, color='red', weight='bold')
    ...
    if 'Completeness (%)' in agg_df.columns and 'Avg Latency (s)' in agg_df.columns:
        ...
        for i, row in avg_perf_df.iterrows():
            ax.text(row['Avg Latency (s)'] * 1.01, row['Completeness (%)'],
                    row['Algorithm'], fontsize=10, va='center')
...
if __name__ == '__main__':
    ...
    OUTPUT_DIR = 'INFOCOM2025_ADAPT_GUAV/showcases/Simulating_a_data_collection_scenario_static/plots_improved'
